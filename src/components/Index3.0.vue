  ...
  <div id="login">
    <div class="login-wrap">
      ...
      <el-form :model="ruleForm" status-icon :rules="rules" ref="loginForm" class="login-form" size="medium">
        ...
        <el-form-item prop="passwords" class="item-from" v-show="model === 'addUser'">
          <label>重复密码</label>
...
//父组件给子组件传值，引入子组件
import ConsoleSon from '../views/back/Console.vue'
export default {
    name: 'index3.0',
    components:{
      ConsoleSon
    ...
    setup(props, { refs, root }){
      ...
      const menuTab = reactive([
        ...
        { txt: '注册', current: false, type: 'addUser' }
      ]);
      // 模块值 
      ...
      const username=ref(ruleForm.username.value,'chuanzhi');
      // 表单的验证
      ...
      const toggleMenu = (data => {
        ...
        resetFromData()  //登录注册相互切换时 清除（重置）表单
        clearCountDown()  //登录注册相互切换时 清除倒计时
      });
      ...
      const getSms = (() => {
        ...
        GetSms(requestData).then(response => {
          ...
          console.log(error,'sendEmail函数 erro');
        })
      });
      ...
      const login = (() => {
        ...
        //登录成功，路由跳转
        root.$store.dispatch('app/login',requestDataJson).then(response=>{
          // console.log("登录成功")
          console.log(response,'登录响应数据')
          root.$router.push({
            // path:'console',
            name:'Console'
          })
...
.login-form {
  ...
  .item-from { margin-bottom: 13px; color: black;}
  .block {
    display: block;
    width: 100%;
  }
  .login-btn { margin-top: 19px;color:black;font-family:"微软雅黑" }
}
</style>
<!--
密码加密：
